...
Wallet.prototype._startWriterWorker = function(callback) {
  var options = {
  ...
  options.listen = this.config.getWriterSocketPath();
...
Wallet.prototype._connectWriterSocket = function(callback) {
  var self = this;
...
Wallet.prototype._startWebWorkers = function(callback) {
  var options = {
    network: this.config.getNetworkName(),
    bitcoinHeight: this.bitcoind.height,
    bitcoinHash: this.bitcoind.tiphash
  ...
  assert(this.config.data.web.port, '"port" option of "web" is expected');
  options.port = this.config.data.web.port;

  var spawnOptions = [path.resolve(__dirname, './web-workers'), JSON.stringify(options)];
