...
WebWorker.prototype._fromRawTransaction = function(txn, wallet, walletId, result) {
  var self = this;
  ...
  function isWalletAddress(address) {
    ...
    if (address && wallet.addressFilter.contains(address)) {
  ...
  function filterOutput(output) {
    ...
    if (output.scriptPubKey && output.scriptPubKey.addresses && output.scriptPubKey.addresses.length === 1) {
      ...
      wallet = isWalletAddress(output.address);
    }
...
/* jshint maxparams: 5 */
WebWorker.prototype._importTransaction = function(txn, wallet, walletId, txid, callback) {
  var self = this;
  ...
  this.clients.getRawTransaction(txid.toString('hex'), 1, function(err, response) {
    ...
    var transaction = self._fromRawTransaction(txn, wallet, walletId, response.result);
...
WebWorker.prototype.getWalletTransactions = function(walletId, options, callback) {
  ...
  this._getLatestTxids(txn, walletId, options, function(err, result) {
    ...
    async.mapSeries(result.txids, function(txid, next) {
      ...
      if (!buffer) {
        self._importTransaction(txn, wallet, walletId, txid, function(err, tx) {
          next(err, tx);
        });
      } else {
