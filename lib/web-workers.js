...
WebWorker.prototype._getLatestTxids = function(txn, walletId, options, callback) {
  ...
  function iterate() {
    cursor.getCurrentBinary(function(key, value) {
      ...
      var pastEnd = utils.isRangeMoreThan(parsedKey, options.end);
      var looped = !utils.isRangeMoreThan(parsedKey, lastParsedKey);
      ...
      if (!pastEnd && !looped && !reachedLimit) {
        iterate();
      } else {
        var result = {};
        var end = setEnd({
    ...
    function setEnd(opts) {
      var MAX_INT = 0xffffffff;
      var end = {};
      if (opts.parsedKey.index === MAX_INT) {
        end.height = opts.parsedKey.height + 1;
        end.index = 0;
      } else {
        end.height = opts.parsedKey.height;
        end.index = opts.parsedKey.index + 1;
      }
      if (utils.isRangeMoreThan(end, opts.options.end)) {
        return {};
      }
      return end;
    }
...
WebWorker.prototype._convertDateToHeight = function(dates, callback) {
  ...
  async.waterfall([
    function(next) {
      ...
      hashDates[1].setUTCHours(0,0,0);
      hashDates[0].setUTCHours(0,0,0);
