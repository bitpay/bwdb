...
WebWorker.prototype._getLatestTxids = function(txn, walletId, options, callback) {
  /*
  how txids are gathered
                  block 100
  -------------------------------------------- <- this is "start"
  | tx index n (most recent tx in the block) | when processing each block:
  | tx  index n -1                           | jump down to index 0, 1 ...
  | tx index 0 (coinbase)                    |
  |-------------------------------------------
                  block 99
  -------------------------------------------- <- processed after block 100
  | tx index n (most recent tx in the block) |
  | tx  index n -1                           |
  | tx index 0 (coinbase)                    |
  |------------------------------------------- <- this is "end"
  */
  ...
  var found = cursor.goToKey(start.getKey());
  if (!found) {
  ...
  function iterator() {
    cursor.getCurrentBinary(function(key, value) {
      ...
      var hasReachedEnd = (options.end && utils.isRangeLessThan(parsedKey, options.end));

      // Make sure that we haven't looped around
      if (lastParsedKey && !utils.isRangeLessThan(parsedKey, lastParsedKey)) {
        hasReachedEnd = true;
      }
      ...
      if (!hasReachedEnd) {
        ...
        hasReachedEnd = !(prevFound && txids.length < options.limit);
      }
      ...
      if (!hasReachedEnd) {
        ...
        // End position is non-inclusive, and can be used as the next start position
        // and only include this option if there are potentially more results
        if (txids.length === options.limit) {
          if (parsedKey.index === 0) {
            result.end = {
              height: parsedKey.height - 1,
              index: MAX_INT
            };
          } else {
            result.end = {
              height: parsedKey.height,
              index: parsedKey.index - 1
            };
          }
        }
        callback(null, result);
...
WebWorker.prototype._updateLatestTip = function() {
  ...
  if (found !== null) {
    ...
    cursor.close();
    txn.abort();
    log.error(new Error('Unable to update tip'));
  }
};
...
WebWorker.prototype._endpointGetHeightsFromTimestamps = function() {
  var self = this;
  return function(req, res) {
    var startdate = new Date(req.query.startdate).getTime() / 1000;
    var enddate = new Date(req.query.enddate).getTime() / 1000;
    ...
    self._convertDateToHeight(startdate, enddate, function(err, blockHeights) {
      if (err) {
      ...
      return res.jsonp({
        result: blockHeights.sort().reverse()
      });
    });
  };
};
...
WebWorker.prototype._convertDateToHeight = function(start, end, callback) {
  var self = this;
  async.waterfall([
    function(next) {
      self.clients.getBlockHashes(start, end, { noOrphans: true }, function(err, response) {
        if (err) {
          return next(utils.wrapRPCError(err));
        }
