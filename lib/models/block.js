...
WalletBlock.fromBuffer = function(keyString, valueBuffer) {
  var keyBuffer = keyString;
  if (!Buffer.isBuffer(keyString)) {
    keyBuffer = new Buffer(keyString, 'hex');
  }
...
WalletBlock.getKey = function(height) {
  var buffer = new Buffer(new Array(4));
  buffer.writeUInt32BE(height);
  return buffer;
};

WalletBlock.prototype.getKey = function() {
  return WalletBlock.getKey(this.height);
};
