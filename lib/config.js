...
Config.prototype.getWriterSocketPath = function() {
  return path.resolve(this.getApplicationPath(), './writer.sock');
};
...
Config.prototype.getURLSync = function() {
  ...
  var url = (config.web.https ? 'https' : 'http') + '://localhost:' + config.web.port;
  return url;
};
