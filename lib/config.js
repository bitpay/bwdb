...
Config.prototype.setConfig = function(data, callback) {
  ...
  // expand relative paths for bitcoind config
  ...
  // expand relative paths for tls options
  if (this.data.wallet && this.data.wallet.tls) {
    assert(this.data.wallet.tls.key, '"wallet.tls.key" file path expected');
    assert(this.data.wallet.tls.cert, '"wallet.tls.cert" file path expected');
    this.data.wallet.tls.key = path.resolve(this.path, this.data.wallet.tls.key);
    this.data.wallet.tls.cert = path.resolve(this.path, this.data.wallet.tls.cert);
  }
...
Config.prototype.hasTLS = function() {
  return !!this.data.wallet.tls;
};

Config.prototype.getTLSOptions = function() {
  return {
    key: fs.readFileSync(this.data.wallet.tls.key),
    cert: fs.readFileSync(this.data.wallet.tls.cert)
  };
};
