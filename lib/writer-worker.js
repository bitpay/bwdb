...
WriterWorker.prototype._removeUTXO = function(txn, walletId, delta) {
  var utxoKeyBuffer = models.WalletUTXO.getKey(walletId, delta.prevTxid, delta.prevIndex);
  ...
  var satKey = models.WalletUTXOBySatoshis.getKey(walletId, utxo.satoshis, delta.prevTxid, delta.prevIndex, 'hex');
  txn.del(this.db.utxosBySatoshis, satKey);

  var heightKey = models.WalletUTXOByHeight.getKey(walletId, utxo.height, delta.prevTxid, delta.prevIndex, 'hex');
  txn.del(this.db.utxosByHeight, heightKey);
};

WriterWorker.prototype._connectBlockAddressUTXODelta = function(txn, walletId, data, delta) {
  if (delta.satoshis > 0) {
    var utxoData = {
  ...
  if (!buffer) {
    ...
    walletIds.forEach(function(walletId) {
      ...
      for (var i = 0; i < data.deltas.length; i++) {
        ...
        // update the utxo
        self._connectBlockAddressUTXODelta(txn, walletId, data, delta);
      }
...
WriterWorker.prototype.sync = function(options, callback) {
  ...
  console.info('Starting sync, height: ' + this.bitcoinHeight + ' hash:', this.bitcoinHash);
  ...
  async.whilst(function() {
    ...
    console.info('Finished sync, height: ' + self.bitcoinHeight + ' hash:', self.bitcoinHash);
    callback();
  });
