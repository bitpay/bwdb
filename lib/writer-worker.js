...
WriterWorker.prototype._sendResponse = function(socket, id, error, result) {
  if (socket) {
    ...
    if (error) {
      console.error('Write task error:', error);
    } else {
      console.info('Completed write task: ' + id);
    }
  }
};
  ...
  if (this.methodsMap[task.method]) {
    ...
    var callback = function(err, result) {
      if (err && err.deferrable && !task.deferred) {
        ...
        console.info('Deferring task id: ' + task.id + ', ' + 'method: ' + task.method + ', error: ' + err.message);
        next();
      } else {
...
WriterWorker.prototype._undoRemoveUTXO = function(txn, walletId, delta, spentOutputs) {
  ...
  var utxo = models.WalletUTXO(spentOutputs[utxoKey.toString('hex')]);
