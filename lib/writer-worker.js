 ...
 * @param {}
 */
WriterWorker.prototype._connectBlockAddressDeltas = function(txn, data, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
WriterWorker.prototype._connectBlockCommit = function(txn, block, callback) {
  var self = this;
  ...
  walletBlock.height = block.__height;
...
WriterWorker.prototype._connectBlock = function(block, callback) {
  ...
  async.each(Object.keys(addressDeltas), function(address, next) {
    self._connectBlockAddressDeltas(txn, {
      address: address,
    ...
    self._connectBlockCommit(txn, block, callback);
  });
};
...
WriterWorker.prototype._addAddressesToWallet = function(txn, walletBlock, walletId, wallet, newAddresses, callback) {
  ...
  var ranges = utils.splitRange(1, walletBlock.height, 25000);
  var queries = [];
