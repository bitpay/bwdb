  ...
  if (this.methodsMap[task.method]) {
    ...
    var callback = function(err, result) {
      if (err && err.deferrable && !task.deferred) {
        task.deferred = true;
        self.queue.push(task, 100);
        log.info('Deferring task id: ' + task.id + ', ' + 'method: ' + task.method + ', error: ' + err.message);
        next();
      } else {
...
WriterWorker.prototype._disconnectTransaction = function(txn, wallets, height, transaction, spentOutputs) {
  var self = this;
...
WriterWorker.prototype._disconnectTip = function(callback) {
  ...
  walletBlock.deltas.forEach(function(transaction) {
    self._disconnectTransaction(txn, wallets, walletBlock.height, transaction, walletBlock.spentOutputs);
  });
