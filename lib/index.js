...
BitcoinWallet.prototype._loadWalletData = function(callback) {
  var self = this;
  this.db.get(models.Wallet.KEY, function(err, buffer) {
    if (utils.isNotFoundError(err)) {
      // wallet is brand new
      // TODO validate that keys and txs is also empty
      var height = self.bitcoind.height;
      var blockHash = self.bitcoind.tiphash;
...
BitcoinWallet.prototype._loadWalletTxids = function(callback) {
  var self = this;
...
BitcoinWallet.prototype._loadWalletDataAll = function(callback) {
  var self = this;
  async.series([
    function(next) {
...
BitcoinWallet.prototype._importTransaction = function(txid, callback) {
  var self = this;
  this.bitcoind.getDetailedTransaction(txid.toString('hex'), function(err, transaction) {
    ...
    self.db.put(tx.getKey(), tx.toBuffer(), function(err) {
      if (err) {
        return callback(err);
      }
      callback(null, tx);
...
BitcoinWallet.prototype.getWalletTransactions = function(options, callback) {
  ...
  var txids = this.walletTxids.getBlockHeightRange(options.start, options.end);

  async.mapLimit(txids, this.defaults.historyConcurrency, function(txid, next) {
    var key = models.WalletTransaction.getKeyFromTxid(txid);
    self.db.get(key, function(err, value) {
      if (utils.isNotFoundError(err)) {
        // The key hasn't been imported yet, so we need to import it
        return self._importTransaction(txid, next);
      } else if (err) {
        return next(err);
      }
      var tx = models.WalletTransaction.fromBuffer(value);
      next(null, tx);
    });
  }, callback);
