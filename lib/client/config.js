...
ClientConfig.prototype.writeApiKey = function(cipherText, salt, callback) {
  this.data.apiKey = {cipherText: cipherText, salt: salt};
  var configFile = this.getConfigFilePath();
  fs.writeFile(configFile, JSON.stringify(this.data, false, 2), callback);
};
  ...
  async.retry(3, function(next) {
    utils.getPassphrase(function(err, passphrase) {
      utils.decryptSecret({
        ...
        var privateKey = bitcore.PrivateKey(secret);
