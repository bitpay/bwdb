...
exports.checkBlockHeightRange = function(blockHeights, req, res, range) {
  if (!(blockHeights[0] >= 0 && blockHeights[1] >= 0)) {
    utils.sendError({
      message: 'No results found for date range',
      statusCode: 404
    }, res);
    return false;
  }

  range.height = blockHeights[0];
  range.index = MAX_INT;
