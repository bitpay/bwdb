...
describe('Wallet Txids Model', function() {
  ...
  describe('#_searchLowerBound', function() {
    ...
    describe('will return found if there is a match', function() {
      it('insert value 2, return match true', function() {
        var txids = new WalletTxids({data: data});
        var position = new Buffer(new Array(8));
        position.writeUInt32BE(0);
        var result = txids._searchLowerBound(position);
    ...
    describe('will find the lower bound position index', function() {
      it('insert value 1, equal index 0', function() {
        ...
        lower.found.should.equal(false);
        lower.index.should.equal(0);
        txids._data[lower.index].readUInt32BE().should.equal(0);
        txids._data[lower.index + 1].readUInt32BE().should.equal(2);
      });
      it('insert value 3, equal index 1', function() {
        ...
        var lower = txids._searchLowerBound(position);
        lower.found.should.equal(false);
        lower.index.should.equal(1);
        txids._data[lower.index].readUInt32BE().should.equal(2);
        txids._data[lower.index + 1].readUInt32BE().should.equal(4);
      });
      it('insert value 1000001, equal index 500000', function() {
        ...
        var lower = txids._searchLowerBound(position);
        lower.found.should.equal(false);
        lower.index.should.equal(500000);
        txids._data[lower.index].readUInt32BE().should.equal(1000000);
        txids._data[lower.index + 1].readUInt32BE().should.equal(1000002);
      });
      it('will quickly find end position if value is greater', function() {
        ...
        var lower = txids._searchLowerBound(position);
        lower.found.should.equal(false);
        lower.index.should.equal(1000000);
        txids._data[lower.index].readUInt32BE().should.equal(2000000);
      });
  ...
  describe('#insert', function() {
    it('will insert into the array', function() {
      ...
      txids._searchLowerBound = sinon.stub().returns({
        found: false,
        index: 2
      });
